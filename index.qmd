...
Les Modèles en Bloc Stochastiques (aussi appelé SBM) sont des modèles probabilistes. Ils supposent que les nœuds d’un même réseau sont divisés en blocs (groupe, clusters) latents regroupant les entités ayant des modèles de connectivité similaires. Latents signifie qu’il n’y a pas d’apriori sur le type de structure recherchée.
...
*load_team_table_csv* est basé sur la construction de *load_team_table_url*. Je l'ai adapté pour pouvoir charger depuis un fichier local en utilisant read.csv
...
Comme précisé dans l'introduction l'entiereté des méthodes informatiques développés ont été réalisé sur Rstudio. J'ai donc travaillé avec le language R. Les packages utilisés sont : faire la liste des packages utilisés... expliquer l'api HAL ?? peut etre en parler dans l'intro
