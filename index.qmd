...
A travers cet exemple, les limitations de la création de réseau sur les données récupérées via un identifiant de laboratoire transparaissent. De fait, on voit l'appartition de cercle de co-auteurs très denses. Ces cercles correspondent généralement à une unique publication contenant de nombreux auteurs. J'ai tout de même réalisé un modèle en bloc stochastique sur ce réseau. Pour cela, j'ai crée une matrice d'adjacence, puis j'ai utilisé cette matrice avec les fonctions codées apr le package SBM pour obtenir mon modèle. Enfin, j'ai ajouté les couleurs aux noeuds en fonction de leurs appartenance aux groupes determiné lors de la création du modèle (voir le code du script en annexe :[Script_R_SBM](#Script-R-SBM-Full-Network)).
