...
Les noeuds sont partitionnés en $K$ groupes latents. On définit un vecteur $Z$ où $Zi = k$ , $k$ est le bloc auquel appartient le noeud $i$ . Dans le cas d'un réseau unipartite, on définit deux probabilités :

-   $P(Zi = k) = \pi_k$ , est la probabilité d'appartenance au groupe $k$.

-   $P(Y_{ij} = 1 | Z_i = k, Z_j = k') = \gamma_{kk'}$ , est la probabilité pour une paire de noeuds appartenant à un bloc $k$ et à un bloc $k'$ d'être en interaction (dans le cas binaire).
