...
```{r exemple utiliation HAL extract csv}
date_min = "01/01/2022"
date_max = "01/01/2023"
...
Afin d'obtenir les informations sous-jacentes des informations bibliographiques, j'ai commencé à travailler à la création d'un réseau de co-auteur. Pour l'instant, la fonctionalité n'existe qu'au format de script R sur un document quarto. Au cours des semaines restantes, l'objectif est de continuer à travailler dessus et créer des fonctions simples et claires permettant à l'utilisateur de créer son propre réseau à partir de ses données. Le script de création de fonction est inspiré d'un script fournie au préalable par ma tutrice Julie Aubert. Suite à mes modifications et mes ajouts, Il se divise en plusieurs parties ((Pour voir le corps de la fonction, veuillez vous référer à l'annexe : [Script_R_reseau_co-auteur](#reseau-co-auteur)) :

La première partie correspond à l'extraction des données et au nettoyage (organisation, séparation) de ces dernières afin qu'elles soient adaptées et permettent de construire un reseau correct. Dans cette partie, l'année de publication est extraite et les identifiants auteurs sont nettoyés. Les informations sur les auteurs sont extraites en colonne distinctes (nom complet, ID de personne, idHal) et convertient en caractères. Les auteurs sont ensuite associés aux structures et les informations sont à nouveau organisé en colonnes. Les données sont ensuite vérifié et les incohérences de noms ou d'identifiants HAL sont corrigées. Les colonnes avec les informations des structures et celles avec les informations des auteurs sont alors fusionnées pour obtenir un ensemble de données complet. Dernièrement, des identifiants fictifs sont crées pour les auteurs sans identifiant.

La deuxième partie concerne la construction et la visualisation du graphe des co-auteurs à partir des données nettoyées et organisées. Dans cette partie, des matrices de contingence sont créées à partir des données des auteurs et des publications. Ces matrices sont ensuite converties en listes de nœuds, représentant les auteurs, et en listes d'arêtes, représentant les collaborations entre les auteurs. Les nœuds se voient attribuer des groupes et des couleurs en fonction des années de publication. Cela permet de visualiser les différentes périodes d'activité des auteurs. Des légendes sont également créées pour indiquer les tailles et les couleurs des nœuds et des arêtes, facilitant ainsi la compréhension du graphe. Ensuite, des informations supplémentaires, telles que des descriptions, sont intégrées pour enrichir les légendes des nœuds et des arêtes. Cela permet d'avoir des détails contextuels supplémentaires lors de la visualisation du graphe. Les informations finales sont ajoutées aux nœuds. Les doublons sont nettoyés et les données sont préparées pour une visualisation cohérente et complète du graphe des co-auteurs. Cette préparation finale assure que toutes les informations nécessaires sont correctement intégrées et prêtes pour être visualisées de manière efficace. Enfin, la visualisation du réseau est réalisé via visnetwork.
