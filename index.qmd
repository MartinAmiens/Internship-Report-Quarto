...
Pour comprendre les choix réalisés lors du développement du package, il est nécessaire de réaliser une introduction aux réseaux. Un réseau d’interaction est constitué de nœuds (les entités) et d’arêtes (les liens entre ces entités). Toute entité peut être modélisée comme un nœud, et leurs interactions comme des arêtes. Les réseaux sont utilisés comme représentation comme objet d'analyse statistique dans de nombreux domaines.Par exemple, Des réseaux de gènes sont réalisés à partir de données de co-expression. Il est possible de donner d'autres exemples comme les analyses de réseaux d'interactions d'espèces qui sont réalisés en écologie (mettre lien vers vignette sbm espece arbre). Les utilisations des réseaux sont nombreuses. Les réseaux se divisent en deux catégories principales (voir figures suivante): les réseaux Unipartites où tous les nœuds sont du même type et les réseaux Bipartites où il y a deux types distincts de nœuds, et les liens ne se forment qu’entre des nœuds de types différents.(voir [exemple_reseau_unipartite_bipartite](#reseau-unipart-bipart) pour le code de réalisation des réseaux d'exemple)

#### Exemple des deux types de réseau
