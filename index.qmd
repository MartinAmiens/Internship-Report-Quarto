...
J'ai commencé par modifier dans le fichier **mia_table.R** la fonction *`load_mia_table`*. A l'origine, cette fonction permettait le chargement d'un csv en ligne fourni par les Ressources Humaines depuis un serveur. Le fichier contient des informations telles que le nom, le prénom, l'idhal, l'équipe, etc, des personnes appartenant au labo. Cette fonction n'était pas générique. Ainsi, il était nécessaire de la modifier. J'ai fait le choix de renommer le fichier **load_table.R**. J'ai remplacé la fonction *`load_mia_table`* par trois fonctions *`load_team_table_csv`*, *`load_team_table_url`*, et *`load_team_table`*. J'ai aussi modifié la majorité des fonctions déjà présentes dans le package faisant appel à *`load_mia_table`*. Les paramètres en entrée des trois fonctions sont les même que la fonction *`load_mia_table`* d'origine avec l'ajout d'un paramètre permettant à l'utilisateur de rentrer l'adresse du fichier. J'ai ensuite créé une documentation via le package {roxygène2} conforme aux critères requis pour un package mis en ligne (veuillez vous référez à l'annexe : [Exemple d'une documentation classique avec Roxygen2](#doc-Roxygen2) ) pour chacune des trois fonctions.
